...
from crewai_tools import FirecrawlSearchTool,FirecrawlScrapeWebsiteTool,GithubSearchTool,SerperDevTool
import os
...
def load_llm():
    return llm
# os.environ["GIT_ACCESS_TOKEN"] =  os.getenv("GIT_ACCESS_TOKEN") 
...
def create_agents_and_tasks(pdf_tool):
    ...
    web_scrap_tool=FirecrawlScrapeWebsiteTool()
    ...
    retriever_agent = Agent(
        ...
        goal=(
            ...
            "If the retrieved result from the PDF search tool does not align with the "
            "input query given to you ({query}), then use the Firecrawl web search tool."
        ),
        backstory=(
        ...
        verbose=True,
        tools=[t for t in [pdf_tool,web_search_tool,web_scrap_tool,tool] if t],
        llm=load_llm()
    )
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        #         full_response += '\n'
        #     message_placeholder.markdown(full_response + "▌")
        #     time.sleep(0.35)  # Adjust the speed as needed
